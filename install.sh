...
uninstall_skill() {
  ...
  # Скилл мог наменять по ходу работы того, о чём установщик не знает: свои хуки,
  # состояние, системные юниты. Кто наменял, тот и убирает — до снятия симлинков,
  # пока команды скилла ещё на месте.
  if [[ -f "$dir/teardown.sh" ]]; then
    if (( DRY )); then
      dim "выполнил бы уборку: teardown.sh"
    else
      bash "$dir/teardown.sh" || warn "уборка скилла не прошла"
    fi
  fi
